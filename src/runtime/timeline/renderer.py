...
class TimelineRenderer:
    ...
    def render_html(timeline: ExecutionTimeline) -> str:
        """Randează Execution Explorer cu Summary, Timeline și Event Details bogate."""
        ...
        html.append("<h3 style=\'margin: 0; color: #111827; font-size: 16px; font-weight: 700;\'>🧭 Execution Explorer & Event Details</h3>")
        ...
        for ev in timeline.events:
            ...
            # Generăm detaliile evenimentului prin EventDetailsBuilder
            details = EventDetailsBuilder.build_for_event(ev.id, ev.event_type, ev.title, ev.metadata)

            html.append("<div style=\'position: relative; margin-bottom: 24px;\'>")
            ...
            html.append("<div style=\'background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 14px;\'>")
            ...
            if ev.duration_ms > 0:
                html.append(f"<div style=\'font-family: monospace; font-size: 11px; background: {st_style['badge']}; color: {st_style['text']}; padding: 3px 6px; border-radius: 4px; display: inline-block; margin-bottom: 8px;\'>Duration: {round(ev.duration_ms, 2)}ms</div>")

            # Randăm secțiunile detaliate (Event Details)
            for section in details.summary_sections:
                html.append(f"<div style=\'background: #ffffff; border: 1px solid #e2e8f0; border-radius: 6px; padding: 10px; margin-top: 8px;\'>")
                html.append(f"<div style=\'font-size: 11px; font-weight: 700; color: #334155; margin-bottom: 6px; text-transform: uppercase;\'>{section.title}</div>")
                html.append("<div style=\'display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px; font-size: 12px; font-family: monospace;\'>")
